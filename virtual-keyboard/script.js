const Keyboard = {
  ...
  _createKeys() {
    ...
    const keyLayout = [
      ...
      "en", "space", "left", "right", 'music', 'speech'
    ];

    //  ";", ":", '"', "'", "<", ">", "/", , 'stop'

    const input = document.querySelector(".use-keyboard-input");
    // console.log(document.dataset.key)
    ...
    document.onkeypress = function (event) {
      document.querySelectorAll('.keyboard__key').forEach(el => el.classList.remove('colorActive'));
      // const currentBtn = document.querySelector(`.${String.fromCharCode(event.keyCode)}`);
      if (event.keyCode === 13) {
        // console.log(event)
    ...
    keyLayout.forEach(key => {
      ...
      switch (key) {
        ...
        case "caps":
          ...
          keyElement.addEventListener("click", () => {
            this._toggleCapsLock();
            keyElement.classList.toggle("keyboard__key--active", this.properties.capsLock); //Когда вторым параметром передано false - удаляет указанный класс, а если true - добавляет
            this.focusMethod();
        ...
        case "shift":
          ...
          keyElement.addEventListener("click", () => {
            this._toggleCapsLock();
            keyElement.classList.toggle("keyboard__key--activeShift"); //Когда вторым параметром передано false - удаляет указанный класс, а если true - добавляет
            this.focusMethod();
          ...
          // console.log(inactiveEnglish)
          // console.log(inactiveEnglish)

          function activeEnglishSym() {
            // console.log(allEnglishSym)
          ...
          // keyElement.addEventListener('mouseup', activeEnglishSym);
          break;
        ...
        case "enter":
          ...
          keyElement.addEventListener("click", () => {
            // this.properties.value = this.properties.value.slice(0, input.selectionStart) + '\n' + this.properties.value.slice(input.selectionStart);
        ...
        case "speech":
          ...
          recognition.interimResults = false; // возвращает последний вариант фразы
          ...
          keyElement.addEventListener("click", () => {
            ...
            // console.log(btnLang.textContent)
